...
export const createSchedule = async (req, res) => {
  const { grade, day, block_schedule_id, classroom_id, teacher_subject_id } =
    req.body;
  ...
  try {
    ...
    const teacherConflict = await Schedule.findOne({
      where: { block_schedule_id, teacher_subject_id, day },
    });
    if (teacherConflict) {
      return res.status(400).json({
        message: "El docente ya tiene asignada una clase en este bloque y día",
      });
    }

    const classroomConflict = await Schedule.findOne({
      where: { block_schedule_id, classroom_id, day },
    });
    if (classroomConflict) {
      return res
        .status(400)
        .json({ message: "El aula ya está ocupada en este bloque y día" });
    }
...
export const updateSchedule = async (req, res) => {
  ...
  try {
    ...
    const currentDay = day || schedule.day;
